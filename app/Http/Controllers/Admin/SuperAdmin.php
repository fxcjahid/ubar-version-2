...
class SuperAdmin extends Controller
{
    //
     ...
     public function index() {
        if (!auth()->user()->can('super-admin-list')) {
     ...
     */
    public function superAdminListAjax(Request $request) {
        ...
        foreach ($users as $user) {
            $data[] = array(
                ...
                '<a href="'.url('admin/edit-super-admin/'.$user->id).'" class="btn btn-primary btn-sm editCity" title="Edit"><i class="fa fa-pencil" ></i></a> | <a href="#" class="btn btn-sm btn-danger  userRemove" data-id="' . $user->id . '" title="Delete"><i class="fa fa-trash"></i></a>'
            );
        }
    ...
    public function create() {
        if (!auth()->user()->can('super-admin-create')) {
			abort(403, 'Unauthorized action.');
		}
        $roles = Role::pluck('name','name')->all();
        return view('admin.super-admin.create',compact('roles'));
    }
    ...
    public function store(Request $request) {
        ...
        if($validator->fails()) {
            ...
            $data = new User();
            $input['unique_id']     = 'SUP91'.rand(100000 , 999999);
            ...
            if($result) {
                return response()->json(['status' =>true , 'message' => 'Super admin added successfully']);
                exit;
            }else{
    ...
    public function statusChange(Request $request) {
        if (!auth()->user()->can('super-admin-edit')) {
			return response()->json(array('status' => false, 'message' => "Opps!! , Dont have Permission"));
                exit;
		}
     ...
     */
    public function reomveSuperAdmin(Request $request) {
        if (!auth()->user()->can('super-admin-delete')) {
			return response()->json(array('status' => false, 'message' => "Opps!! , Dont have Permission"));
                exit;
		}
        try {
            $where = array('id' => $request->userId);
            $delete = User::where($where , ['status' => 'DEACTIVE'])->delete();
            if ($delete) {
                return response()->json(array('status' => true, 'message' => "Super Admin deleted successfully"));
                exit;
            } else {
                return response()->json(array('status' => false, 'message' => "Opps!! , Something went wrong"));
                exit;
    ...
    public function edit($managerId) {
        ...
        return view('admin.super-admin.edit' , compact('manager','roles','userRole'));
    }
    ...
    public function update(Request $request) {
        ...
        if($validator->failed()) {
            ...
            if($update) {
                return response()->json(['status' => true , 'message' => 'Super Admin updated successfully']);
                exit;
            }else{
                return response()->json(['status' => false , 'message' => "Something went wrong try again later !!!"]);
                exit;
