...
class Manager extends Controller
{
     ...
     */
    public function managerListAjax(Request $request) {
        ...
        foreach ($users as $user) {
            $data[] = array(
                ...
                '<a href="'.url('admin/edit-manager/'.$user->id).'" class="btn btn-primary btn-sm editCity" title="Edit"><i class="fa fa-pencil" ></i></a> | <a href="#" class="btn btn-sm btn-danger  userRemove" data-id="' . $user->id . '" title="Delete"><i class="fa fa-trash"></i></a>'
            );
        }
    ...
    public function store(Request $request) {
        ...
        if($validator->fails()) {
            ...
            $data = new User();
            $input['unique_id']     = 'MG101'.rand(100000 , 999999);
            ...
            if($result) {
                return response()->json(['status' =>true , 'message' => 'Manager added successfully']);
                exit;
            }else{
     ...
     */
    public function reomveManager(Request $request) {
        ...
        try {
            ...
            if ($delete) {
                return response()->json(array('status' => true, 'message' => "Manager deleted successfully"));
                exit;
            } else {
                return response()->json(array('status' => false, 'message' => "Opps!! , Something went wrong"));
                exit;
    ...
    public function edit($managerId) {
        ...
        return view('admin.manager.edit' , compact('manager','roles','userRole'));
    }
    ...
    public function update(Request $request) {
        ...
        if($validator->failed()) {
            ...
            if($update) {
                return response()->json(['status' => true , 'message' => 'Manager data updated successfully']);
                exit;
            }else{
                return response()->json(['status' => false , 'message' => "Something went wrong try again later !!!"]);
                exit;
