            ...
            <div class="col-md-12">
                <div class="page_title ">
                    ...
                    <span><a href="{{ route('admin.cityagent') }}"><i
                                class="fa fa-arrow-left black"></i>Back</a>&nbsp;&nbsp;&nbsp;<h2>Edit City Agent</h2></span>
                </div>
            </div>
        </div>
        <!-- row -->
                ...
                <div class="white_shd full margin_bottom_30">
                    <div class="full graph_head">
                        <div class="heading1 margin_0">
                            <h2>Edit City Agent</h2>
                        </div>
                    </div>
                    <div class="padding_infor_info">
                        <form id="update-manager" method="POST">
                            <div class="row">
                                ...
                                <div class="col-xs-6 col-sm-6 col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="city_id" class="form-control" required id="">
                                            ...
                                            <option {{$city->id == $manager->city_id ? 'selected':''}} value="{{$city->id}}">{{$city->city}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
...
<script>
  ...
  no_results_text: "Oops, nothing found!"
    ...
    <script>
        $(function() {
            $('#update-manager').on('submit', function(e) {
                e.preventDefault();
                let fd = new FormData($("#update-manager")[0])
                fd.append('_token', "{{ csrf_token() }}");
                $.ajax({
                    url: "{{ route('admin.cityagent.update') }}",
                    type: "POST",
                    data: fd,
                    dataType: 'json',
                    processData: false,
